...
It is easy to compute $p(\mathbf{x})$ from $\log(p(\mathbf{x}))$ and vice versa, but computing the prior density from $\nabla \log(p(\mathbf{x}))$ can be very expensive as it requires solving a high-dimensional partial differential equation. For this reason, we want to classify priors differently based on the easy accessibility of the prior density. The former we call **explicit priors** with all others being **implicit priors**.
